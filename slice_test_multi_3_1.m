...
%         w32 = slice_decode(y32, length(w3), k, n(1 : 16), 8);
%         w33 = slice_decode(y33, length(w3), k, n(1 : 16), 8);
% 
...
%         w42 = slice_decode(y42, length(w4), k, n(1 : 32), 8);
%         w43 = slice_decode(y43, length(w4), k, n(1 : 32), 8);
% 
...
%         w53 = slice_decode(y53, length(w5), k, n(1 : 32), 8);
        result{t} = result{t} + [sum(w1 == w11) sum(w2 == w21);
                                 sum(w1 == w12) sum(w2 == w22);
                                 sum(w1 == w13) sum(w2 == w23)];
        if mod(i, 50) == 0
            disp([t i]);
            disp(bsxfun(@rdivide, result{t},[num_watermark, num_watermark] * i) * 100);
        end
   end
end
ret = bsxfun(@rdivide, netsum(result), [num_watermark, num_watermark] * (count * times)) * 100;
disp(ret);
save('/home/wujing/hupeng/slice_test_multi_3_1.mat');
